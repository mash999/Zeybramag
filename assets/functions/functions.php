<?php namespace magazine\fetch_functions;
//error_reporting(0);
use magazine\fetch_functions;
session_start();
$base_url= "http://localhost:8080/freelancing/magazine";
$site_name = "Zeybra Magazine";
...
function post_time($publish_time){
	$time_diff = time() - $publish_time;
			...
			if($day < 1){
				if($hour < 1){
					if($min < 1){
						...
						$time = floor($min);
						if($time == 1) $time = $time . " minute";
						else  $time = $time . " minutes";
					}
				}
				else{
					$time = floor($hour);
					if($time == 1) $time = $time . " hour";
					else  $time = $time . " hours";
				}
			}
			else{
				$time = floor($day);
				if($time == 1) $time = $time . " day";
				else  $time = $time . " days";
			}
		}
		else{
			$time = floor($month);
			if($time == 1) $time = $time . " month";
			else  $time = $time . " months";
		}
	}
	else{
		$time = floor($year);
		if($time == 1) $time = $time . " year";
		else  $time = $time . " years";
	}
